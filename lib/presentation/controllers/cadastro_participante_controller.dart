...
class CadastroParticipanteController extends GetxController {
  ...
  Future<int?> createParticipante(
    ...
    int? newParticipanteId =
        await _repository.createParticipante(novoParticipante);
    ...
    if (newParticipanteId != null) {
      ...
      if (newAvaliacaoId != null) {
        ...
        List<ModuloEntity> modulos =
            createModulosEntities(selectedModulos, newAvaliacaoId);
  ...
  List<ModuloEntity> createModulosEntities(
      List<String> selectedActivities, int evaluationID) {
  ...
  void printFormData() {
    ...
    print(
        'Sexo: ${selectedSexo.value == Sexo.homem ? 'Homem' : (selectedSexo.value == Sexo.mulher ? 'Mulher' : 'Not Selected')}');
    print(
        'Escolaridade: ${selectedEscolaridade.value?.toString().split('.').last ?? 'Not Selected'}');
    print('Date: ${selectedDate.value?.toIso8601String() ?? 'Not Selected'}');
    print(
        'Lateralidade: ${selectedLateralidade.value?.toString().split('.').last ?? 'Not Selected'}');
    print(
        'Idioma: ${selectedIdioma.value?.toString().split('.').last ?? 'Not Selected'}');
  }
}
